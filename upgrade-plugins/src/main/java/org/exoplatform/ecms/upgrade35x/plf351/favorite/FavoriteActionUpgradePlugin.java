 ...
 */
package org.exoplatform.ecms.upgrade35x.plf351.favorite;
...
public class FavoriteActionUpgradePlugin extends UpgradeProductPlugin {
  ...
  public void processUpgrade(String oldVersion, String newVersion) {
    ...
    try {
      ...
      try {
        Item item = session.getItem(scriptPath);
        item.remove();
        session.save();
      }
      finally {
        ...
        scriptService.addScript("ecm-explorer/action/" + FILE_NAME_ADD_TO_FAVORITE_ACTION, FILE_NAME_ADD_TO_FAVORITE_ACTION, scriptContent, sessionProvider);
      }
...
//      List<User> userList = WCMCoreUtils.getAllElementsOfListAccess(userListAccess);
      ...
      try {
        actionServiceContainer.init();
      } catch (Exception e) {
        LOG.info("Can not init Action", e);
      }
    }
  }
